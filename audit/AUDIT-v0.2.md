...
## 7b. Changes since this audit — **FeeReceiver (reviewed separately)**

`src/treasury/FeeReceiver.sol` (70% treasury / 30% `$DCA` buyback-and-burn fee sink, the vaults' `feeRecipient` from `Deploy.s.sol` on) was added after v0.3 and has its own report: [`audit/AUDIT-FeeReceiver.md`](AUDIT-FeeReceiver.md) — no High / Medium; 6 design weaknesses found and fixed during the review, 3 Low residuals accepted with operational mitigations; 47 unit + 3 integration tests and a 5-invariant handler suite, green at CI depth. Vault / router code unchanged.
