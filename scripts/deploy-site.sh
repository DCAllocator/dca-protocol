...
# X, and /app redirects to the landing (NEXT_PUBLIC_PRELAUNCH, lib/prelaunch.ts).
#
#   pnpm deploy:site
...
TWITTER_URL="${TWITTER_URL:-https://x.com/DCAllocator}"
GITHUB_URL="${GITHUB_URL:-https://github.com/DCAllocator/dca-protocol}"
...
BUILD_ENV=(
  ...
  -b "NEXT_PUBLIC_TWITTER_URL=${TWITTER_URL}"
  -b "NEXT_PUBLIC_GITHUB_URL=${GITHUB_URL}"
)
...
for v in NEXT_PUBLIC_DIRECTORY NEXT_PUBLIC_CLAIM_HELPER NEXT_PUBLIC_ZAP NEXT_PUBLIC_KEEPER NEXT_PUBLIC_RH_RPC \
  NEXT_PUBLIC_BUY_DCA_URL NEXT_PUBLIC_PONS_URL NEXT_PUBLIC_BRIDGE_URL NEXT_PUBLIC_DISCORD_URL NEXT_PUBLIC_TELEGRAM_URL \
  NEXT_PUBLIC_WC_PROJECT_ID NEXT_PUBLIC_BLOCKED_COUNTRIES; do
  [[ -n "${!v:-}" ]] && BUILD_ENV+=(-b "$v=${!v}")
done
