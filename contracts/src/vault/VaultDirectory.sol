...
/// @notice Single discovery point for frontends and keepers: the three frequency vaults plus shared infra.
/// @dev The vaults themselves are deployed by `script/Deploy.s.sol`. An on-chain factory embedding three
...
contract VaultDirectory is Ownable2Step {
    ...
    /// @notice The three vaults in [daily, weekly, monthly] order.
    function vaults() external view returns (address[3] memory) {
        return [_entry.daily, _entry.weekly, _entry.monthly];
    }
}
