...
- `/` — the landing (`src/app/page.tsx`, sections in `src/components/site/v3/`): sticky nav with the `$DCA` contract chip and a mobile menu; a hero with an animated copy of My plans (a buy fires, Standard vs `$DCA` holder delivery); a live stock tape and figures read from the vaults (hidden until loaded; volume cells only past $25k of stock bought); three steps with a plan-sentence builder ("Buy $50 of NVDA every week.") that deep-links into Create; stock tiles; the `$DCA` section (holder perks, the fee loop); Boost; a trust strip; the FAQ (with FAQPage JSON-LD); a closing CTA band; a footer with the core contracts once deployed; a sticky mobile CTA bar; and the site's share card (`src/app/opengraph-image.tsx`). Copy rules: no fee percentages, no buyback amounts or timing, wallet delivery only for `$DCA` holders. Optional env: `NEXT_PUBLIC_SITE_URL` (absolute share-card URLs; set it in production), `NEXT_PUBLIC_PONS_URL`, `NEXT_PUBLIC_BRIDGE_URL`, and `NEXT_PUBLIC_CHART_URL` (used only once `SHOW_CHART_LINK` in `components/site/v3/config.ts` is on). The earlier product-first page stays at `/legacy`.
- `/app` — sidebar shell (logo, **Create new plan** CTA, a **Buy $DCA** button, My plans, Activity; *Protocol*: Overview, DCA Token, Docs; *Vaults*: USDG waiting to buy per vault, boosted included), wallet button top-right, centred content column topped by a **stock ticker tape** (`components/app/StockTicker.tsx`: the registry's stocks by live market cap with price and 24 h change, scrolling at a constant px/s, paused on hover, static under reduced motion; `w-full`, so it fits whatever parent it is dropped into). Live market data comes from **`/api/stock-market`**, a route that proxies CoinGecko's "Robinhood Chain Stocks Ecosystem" category (one upstream call a minute, cached; optional `COINGECKO_API_KEY` demo key) — display only, nothing sent on chain depends on it. Pages:
  ...
  - **Buy $DCA** — `/app/buy`. An in-app swap into `$DCA` over `AggregatorRouter`, paid in USDG or ETH (a `USDG ▾ / ETH ▾` pill) and smart-routed whichever token `$DCA` is paired with. `readBestRoute` (`hooks/useProtocol.ts`) weighs the router's own `quote()` — a direct hop, or two hops *through* WETH — against the routes *through USDG* it never searches (only an ETH buy can use them): the first leg `quote(WETH, USDG)` joined to the router's onward `quote(USDG, $DCA)` pick and to each approved direct USDG → `$DCA` hop (`approvedHops`), kept only at ≤ 3 hops with no token visited twice, then checked end to end with `quotePath` (the router's impact cap on the whole path). It takes the one that delivers the most: paired with USDG, USDG → `$DCA` and ETH → WETH → USDG → `$DCA`; paired with WETH, ETH → WETH → `$DCA` and USDG → WETH → `$DCA`; with both pools, whichever pays more at that amount. The quote is for the real amount; the path is frozen at click time and sent as `swapWithRoute(payToken, $DCA, amount, minOut, you, path)`, minOut = quote − 0.5%, so the **Route** line (card and dialog) is the route sent and one floor guards it end to end. USDG: Approve USDG (skipped when covered) → Buy. ETH: Approve WETH (skipped when covered) → Wrap ETH → Buy; HALF / MAX keep 0.01 ETH back for gas. **Try again** re-quotes first and never sends below the floor last shown (after a price move it shows the new figures and asks again). An amount with no quote reads "Try a smaller amount" only when the pay token routes at the probe size; one with no route at all says so and points to the other ("No route from USDG to mDCA on this chain — pay with ETH instead."), and the card opens on the pay token the page's probe answered for. Only ERC-20 pools on the approved adapters are routable: a native-ETH Uniswap v4 pool (a Pons launch pool) is not, with either pay token (the UniV4 adapter takes ERC-20/ERC-20 pool keys only). When there is no route and `NEXT_PUBLIC_BUY_DCA_URL` is an https URL, a **Buy on Pons** hand-off; otherwise "No route to $DCA on this chain yet." The tab lights on 31337, with `NEXT_PUBLIC_ENABLE_BUY_TAB=1`, or when the router has a route in for 1 USDG (failing that, 0.001 WETH) — a native-ETH Pons pool will not light it without the flag or a WETH (ERC-20) route. Both variables are in `apps/web/.env.local.example`; `pnpm --filter @dca/web env:local` writes them as commented lines.
...
- Social icons (X, Discord, Telegram, GitHub) in the landing nav and footer and the app sidebar come from `src/lib/socials.ts`, where the URLs are written in the code (they are public and the same in every deployment, so no env vars). On chain 31337 (`pnpm dev`, `pnpm share`, both forks) a link without a URL falls back to the platform's home page as a placeholder; on Robinhood Chain it is hidden, and with none set the icon row is not rendered.
- **Prelaunch mode** (`NEXT_PUBLIC_PRELAUNCH=1`, `src/lib/prelaunch.ts`): for a site that is live before the contracts and the token. Every link into the app (`/app/*`) and to the token's off-site pages (Pons, chart) opens a "Not launched yet" dialog (`components/site/PrelaunchGate.tsx`, mounted in the root layout) that points to X instead of navigating; in-page anchors and community links still work. The middleware sends any `/app` request back to `/?prelaunch`, which opens the same dialog. **Deploying the website:** `pnpm deploy:site` (`scripts/deploy-site.sh`) makes a production Vercel deployment of the Robinhood Chain build in prelaunch mode (`SITE_URL` overrides the site URL); at launch run it with `PRELAUNCH=0` and the `NEXT_PUBLIC_*` contract addresses. `pnpm share` deploys to the same Vercel project and replaces the site, so redeploy with `pnpm deploy:site` after a sharing session. The project is also connected to the GitHub repo, so every push to `main` (including the daily market-cap snapshot commit) rebuilds production with the env vars stored in the Vercel project: `NEXT_PUBLIC_CHAIN=robinhood`, `NEXT_PUBLIC_PRELAUNCH=1` and `NEXT_PUBLIC_SITE_URL` must be set there for Production, or that build is the local-chain site.
- Geo-block: `src/middleware.ts` redirects `/app/*` to `/restricted` for `NEXT_PUBLIC_BLOCKED_COUNTRIES` (default US, GB, CA, AU, CU, IR, KP, SY) using the CDN country header (`x-vercel-ip-country` / `cf-ipcountry`). Contracts stay permissionless.
