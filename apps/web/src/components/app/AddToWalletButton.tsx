...
/**
 * "Add to wallet": asks the connected wallet to track a Stock Token (EIP-747 `wallet_watchAsset`) so the stock a plan
 * buys shows up in the user's token list.
 *
 * Renders nothing unless a wallet is connected on the active chain (the same guard as ConnectButton: a token address
 * only means something on the chain it lives on), and nothing for the local test wallet, whose mock connector answers
 * `true` without any real wallet having been updated — showing a success there would be a lie.
 *
...
export function AddToWalletButton({ address, symbol, decimals, className = "" }: { address: Address; symbol: string; decimals: number; className?: string }) {
  ...
  if (!isConnected || chainId !== activeChain.id || connector?.id === TEST_WALLET_ID) return null;
  ...
  const onClick = async () => {
    ...
    try {
      ...
      if (added) toast({ kind: "ok", title: `Added ${symbol} to your wallet` });
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={isPending}
      title={`Track ${symbol} in your wallet`}
      className={`inline-flex items-center gap-1 text-[11.5px] font-medium text-ink-3 transition-colors hover:text-ink disabled:opacity-60 ${className}`}
    >
